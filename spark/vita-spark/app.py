...
def makeDay(db, file, userId):
    with db.connect() as conn:
        ...
        conn._cursor_execute("DELETE FROM daily_sleep WHERE daily_sleep_start = '" + sleep_date + "'")
        conn._cursor_execute("DELETE FROM daily_wearable WHERE date = '" + day_date + "'")
...
def upload(userId):
    ...
    with db.connect() as conn:
        ...
        conn._cursor_execute("DELETE FROM weekly_wearable WHERE date = '" + week_date + "'")
        conn._cursor_execute("DELETE FROM monthly_wearable WHERE date = '" + month_date + "'")
        conn._cursor_execute("DELETE FROM user_average WHERE user_id = '" + userId + "'")
